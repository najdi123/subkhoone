...
function Primary({propertyData, primaryBuyOffersProps, primarySellOffersProps}) {
    const {data} = useContext(DataContext);

    const [primaryBuyOffers, setPrimaryBuyOffers] = useState()
    const [primarySellOffers, setPrimarySellOffers] = useState()
    useEffect(() => {
        setPrimaryBuyOffers(primaryBuyOffersProps)
        setPrimarySellOffers(primarySellOffersProps)
    ...
    const [inputRange, setInputRange] = useState({
        value: propertyData["present_primary_market"]["low_price"],
    })
                            ...
                            <div className={styles.popupRange}>
                                <div className={styles.inputRange}>
                                    ...
                                    <InputRange
                                        ...
                                        maxValue={propertyData["present_primary_market"]["high_price"]}
                                        minValue={propertyData["present_primary_market"]["low_price"]}
                    ...
                    <div className="position-sticky">
                        <div className={styles.texts}>
                            <div className={styles.line}>
                                <div>
                                    <p className="pl-3">
                                        {moment(propertyData["present_primary_market"]["start_date_time"]).format('jYYYY/jM/jD')}
                                    </p>
                                </div>
                        ...
                        <Carousel>
                            {
                                ...
                                <div>
                                    <img
                                        src={image1}
                                    />
                                    <span className={styles.slicBadge}>
                                        بازار اولیه
                                    </span>
                                </div>
                            }
                        ...
                        <div className={`row ${styles.border}`}>
                            <div className={`col-lg-6 col-12 ${styles.buy}`}>
                                ...
                                <table className="table table-striped mt-4 text-center">
                                    ...
                                    {
                                        primarySellOffers && primarySellOffers.map(item => {
                                                return (
                            ...
                            <div className={`col-lg-6 col-12 ${styles.sell}`}>
                                ...
                                <table className="table table-striped mt-4 text-center">
                                    ...
                                    <tbody>
                                    {
                                        primaryBuyOffers && primaryBuyOffers.map(item => {
                                                return (
                                                    <tr key={item.id} className={styles.tableContent}>
                                                        <td>{item["price"]}</td>
                                                        <td> صاب {item["number_of_shares"]}</td>
...
Primary.getInitialProps = async (ctx) => {
    ...
    const primarySellOffers = await axios.get(`http://api.subkhoone.com/api/assets/${id}/primary_markets/${primaryId}/primary_offers`);
    return {
        propertyData: res.data.data,
        primaryBuyOffersProps: primaryBuyOffers.data.data,
        primarySellOffersProps: primarySellOffers.data.data
    }
};
